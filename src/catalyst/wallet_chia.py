    ...
    try:
        ...
        if result and result.get("success"):
            ...
            return {
                ...
                "peak_height": state.get("peak", {}).get("height", 0)
                if state.get("peak")
                else 0,
            }
            ...
            peers.append(
                {
                    ...
                    "type": c.get(
                        "type", 0
                    ),  # 1=full_node, 2=harvester, 3=farmer, 4=timelord, 5=wallet
...
def split_coins_bulk(
    ...
    for i, rec in enumerate(coin_records[:3]):  # Show first 3
        ...
        print(
            f"      Coin {i}: amount={amt} ({amt / 1e12:.4f} XCH), spent_block={spent}"
        )
    ...
    if not unspent_records:
        ...
        return {
            "success": False,
            "error": "All coins pending spend - retry after confirmation",
        }

    print(
        f"   📊 Found {len(unspent_records)} unspent coins (filtered from {len(coin_records)} total)"
    )
    ...
    if coin_id:
        ...
        amount_bytes = (
            amount.to_bytes(byte_count, "big", signed=True) if amount > 0 else b"\x00"
        )
    ...
    if is_cat:
        ...
        print(
            f"   📊 Largest CAT coin: {coin_amount_tokens:.2f} tokens ({coin_amount} mojos)"
        )
        print(
            f"   🎯 Target: {num_coins} coins × {token_amount:.2f} tokens = {num_coins * token_amount:.2f} total"
        )
        ...
        if coin_amount_tokens >= reserve_amount:
            print(
                f"   💎 Will keep {coin_amount_tokens - needed:.2f} tokens as reserve for future offers!"
            )
        else:
        ...
        print(
            f"   🎯 Target: {num_coins} coins × {coin_size_mojos / 1e12:.4f} XCH = {total_needed_xch:.4f} total"
        )
        ...
        if coin_amount >= reserve_mojos:
            ...
            print(
                f"   💎 Will keep {reserve_xch:.4f} XCH as reserve for future offers!"
            )
        else:
...
def wait_for_coin_confirmations(
    ...
    while (time.time() - start_time) < max_wait_seconds:
        confirmed = count_suitable_coins(
            wallet_id, target_coin_size_mojos, tolerance=tolerance
        )
...
def get_offer_bech32(trade_id: str) -> str:
    ...
    """
    res = rpc("get_offer", {"trade_id": trade_id, "file_contents": True}, timeout=10)
...
def classify_offers_from_list(offers_list: list, asset_id_mz: str):
    ...
    for tr in offers_list:
        ...
        status_label = (
            STATUS_NAMES.get(status_val, str(status_val))
            if isinstance(status_val, int)
            else str(status_val)
        )
