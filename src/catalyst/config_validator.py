...
def validate_config(cfg) -> ValidationReport:
    ...
    if not cat_id:
        err(
            "CAT_ASSET_ID",
            "CAT asset ID is empty — bot cannot identify which token to trade",
        )
    ...
    if not enable_buy and not enable_sell:
        err(
            ...
            "Both buy and sell are disabled — bot has nothing to do",
        )
    ...
    if min_trade > Decimal("0") and max_trade > Decimal("0") and min_trade > max_trade:
        err(
            "MIN_TRADE_XCH/MAX_TRADE_XCH",
            f"MIN_TRADE_XCH ({min_trade}) > MAX_TRADE_XCH ({max_trade})",
        )
    if (
        not tier_enabled
        and default_trade > Decimal("0")
        and max_trade > Decimal("0")
        and default_trade > max_trade
    ):
        warn(
            "DEFAULT_TRADE_XCH",
            f"DEFAULT_TRADE_XCH ({default_trade}) exceeds MAX_TRADE_XCH ({max_trade})",
        )
    ...
    if getattr(cfg, "DYNAMIC_SPREAD_ENABLED", False):
        ...
        if min_sp > Decimal("0") and max_sp > Decimal("0") and min_sp > max_sp:
            err(
                ...
                f"MIN_SPREAD_BPS ({min_sp}) > MAX_SPREAD_BPS ({max_sp})",
            )
    ...
    try:
        ...
        strategy = (
            ...
            if raw_strategy and isinstance(raw_strategy, str)
            else ""
        )
    ...
    try:
        ...
        if raw_weight is not None and isinstance(
            raw_weight, (int, float, str, Decimal)
        ):
    ...
    if expiry < 300:
        err(
            "OFFER_EXPIRY_SECS",
            f"OFFER_EXPIRY_SECS={expiry} is dangerously short (< 5 min)",
        )
    ...
    if refresh_before >= expiry and expiry > 0:
        warn(
            ...
            f"OFFER_REFRESH_BEFORE ({refresh_before}) >= OFFER_EXPIRY_SECS ({expiry})",
        )
    ...
    if total_offers > 300:
        warn(
            "MAX_ACTIVE_*_OFFERS",
            f"Total max offers ({total_offers}) > 300 — wallet may struggle",
        )
    ...
    if requote_cooldown < 10:
        warn(
            ...
            f"REQUOTE_COOLDOWN_SECS={requote_cooldown} is very aggressive (< 10s)",
        )
    ...
    if loop_secs < 30:
        warn(
            "LOOP_SECONDS",
            f"LOOP_SECONDS={loop_secs} is very fast (< 30s) — may cause wallet contention",
        )
    ...
    if tier_enabled:
        ...
        if tier_counts == 0:
            warn(
                "TIER_*_COUNT",
                "TIER_ENABLED=True but all tier counts are 0 — no tiered offers will be created",
            )
    ...
    if getattr(cfg, "SNIPER_ENABLED", False):
        ...
        if sniper_size <= Decimal("0"):
            warn(
                "SNIPER_SIZE_XCH",
                f"SNIPER_ENABLED=True but SNIPER_SIZE_XCH={sniper_size}",
            )
    ...
    if (
        default_trade > Decimal("0")
        and min_trade > Decimal("0")
    ...
    if (
        boost_size > Decimal("0")
        and max_trade > Decimal("0")
        and boost_size > max_trade
    ):
        warn(
            "BOOST_SIZE_XCH",
