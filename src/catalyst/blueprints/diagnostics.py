...
def api_config_export_env():
    ...
    try:
        ...
        emitted = set()
        export_aliases = {
            # The GUI writes MAX_ACTIVE_BUY/MAX_ACTIVE_SELL to .env, while
            # Config exposes the normalized runtime attributes with _OFFERS.
            "MAX_ACTIVE_BUY": "MAX_ACTIVE_BUY_OFFERS",
            "MAX_ACTIVE_SELL": "MAX_ACTIVE_SELL_OFFERS",
        }
        ...
        for section_name, keys in sections:
            ...
            for key in keys:
                ...
                if val is None and key in export_aliases:
                    val = getattr(cfg, export_aliases[key], None)
                if val is None:
                    continue
