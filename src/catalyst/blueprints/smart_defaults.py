...
def _prep_count_from_budget(
    pool_target_xch: float, coin_size_xch: float, cap: int, minimum: int
) -> int:
...
def _smart_fee_prep_count(
    avail_xch: float, fee_coin_size_xch: float, fee_pct: float = 0.03
) -> int:
    ...
    if avail_xch < 3:
        ...
        cap, minimum = 50, 5
    return _prep_count_from_budget(
        avail_xch * max(0.0, _smart_float(fee_pct)), fee_coin_size_xch, cap, minimum
    )
...
def _smart_initial_max_position(
    ...
    if _smart_float(position_mult, 1.0) != 1.0:
        max_position = max(
            floor, round(max_position * _smart_float(position_mult, 1.0), 1)
        )
    return max_position
                ...
                if resp.status_code == 200:
                    ...
                    if tickers:
                        ...
                        if tk_bid > 0 and tk_ask > 0 and tk_bid <= tk_ask:
                            ...
                            print(
                                f"[PRICE_STANDALONE] Dexie bid/ask mid={price:.10f} "
...
def _fetch_dexie_orderbook_standalone(asset_id: str) -> dict:
    ...
    try:
        ...
        result["has_data"] = result["api_ok"] and (
            result["num_buy_offers"] > 0 or result["num_sell_offers"] > 0
        )
        _state_tag = (
            "ok"
            if result["api_ok"]
            and (result["num_buy_offers"] or result["num_sell_offers"])
            else ("empty-book" if result["api_ok"] else "api-failed")
        )
        print(
            f"[SMART_DEFAULTS] Orderbook [{_state_tag}]: "
...
def api_smart_defaults():
    ...
    try:
        ...
        liquidity_mode = (
            (request.args.get("liquidity_mode", "two_sided") or "two_sided")
            .strip()
            .lower()
        )
        if liquidity_mode not in ("two_sided", "buy_only", "sell_only"):
            liquidity_mode = "two_sided"
        dbx_cap = (request.args.get("dbx_cap", "false") or "").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        return jsonify(
            {"error": "Smart Settings calculation failed", "code": "SERVER_ERROR"}
        ), 500
...
def _calculate_smart_defaults(
    ...
    asset_id = (
        (str(asset_id).strip() if asset_id else "")
        or active_cat.get("asset_id")
        or (cfg.CAT_ASSET_ID if hasattr(cfg, "CAT_ASSET_ID") else "")
    )
    decimals = (
        cat_decimals
        if cat_decimals is not None
        else active_cat.get("decimals") or getattr(cfg, "CAT_DECIMALS", 3)
    )
    ticker_id = (
        (str(cat_ticker_id).strip() if cat_ticker_id else "")
        or active_cat.get("ticker_id")
        or (cfg.CAT_TICKER_ID if hasattr(cfg, "CAT_TICKER_ID") else "")
    )
    cat_wid = (
        cat_wallet_id or active_cat.get("wallet_id") or getattr(cfg, "CAT_WALLET_ID", 2)
    )
    cat_name = (
        (str(cat_name).strip() if cat_name else "")
        or active_cat.get("name")
        or getattr(cfg, "CAT_NAME", "CAT")
    )
    if ticker_id and "_" not in ticker_id:
        ticker_id = f"{ticker_id}_XCH"
    ...
    try:
        ...
        if xr and xr.get("success"):
            ...
            if _raw_total <= 0:
                _raw_total = api_server._safe_float(
        ...
        if has_wallet:
            ...
            print(
                f"[SMART_DEFAULTS v2] Wallet (total): {xch_spendable:.4f} XCH, {cat_spendable:.0f} CAT"
            )
    ...
    try:
        ...
        print(
            "[SMART_DEFAULTS v2] Cleared market analysis cache for fresh data (kept Spacescan cache)"
        )
    except Exception:
    ...
    print(
        f"[SMART_DEFAULTS v2] === Calculating settings (data quality: {quality.get('quality', '?')}) ==="
    )
    ...
    if fills_per_day == 0 and daily_volume == 0 and ticker_volume_30d > 0:
        ...
        print(
            f"[SMART_DEFAULTS v2] Using ticker 30d volume fallback: "
    ...
    if fills_per_day > 10 and daily_volume > 5:
        ...
        spread_base = 300  # 3%
        messages.append(
            f"Active market: {fills_per_day:.0f} fills/day, {daily_volume:.1f} XCH/day → tighter spread"
        )
    elif fills_per_day > 3 and daily_volume > 1:
        # Moderate market
        spread_base = 500  # 5%
        messages.append(
            f"Moderate market: ~{fills_per_day:.1f} fills/day, {daily_volume:.1f} XCH/day → balanced spread"
        )
    elif fills_per_day > 0.5 or daily_volume > 0.1:
        # Quiet market — profit per trade matters more
        spread_base = 700  # 7%
        messages.append(
            f"Quiet market: ~{fills_per_day:.1f} fills/day, {daily_volume:.2f} XCH/day → wider spread"
        )
    elif ticker_volume_30d > 0:
        # Very low volume but ticker shows SOME activity
        spread_base = 700  # 7%
        messages.append(
            f"Low volume: {ticker_volume_30d:.2f} XCH in 30 days → wider spread"
        )
    else:
    ...
    if _rp["spread_bps_mult"] != 1.0:
        base_spread_bps = max(
            250, min(1000, round(base_spread_bps * _rp["spread_bps_mult"]))
        )

    print(
        f"[SMART_DEFAULTS v2] Spread: {spread_base} base + {vol_adj} vol({regime}) + "
        ...
        + (
            f" [×{_rp['spread_bps_mult']} {_risk_profile_name}]"
            if _rp["spread_bps_mult"] != 1.0
            else ""
        )
    )
    ...
    band_basis = (
        range_90d_pct
        if (quiet_phase and range_90d_pct > range_30d_pct)
        else range_30d_pct
    )
    if regime == "extreme":
        dynamic_limit_pct = max(
            100, round(band_basis * 1.5 / 5) * 5
        )  # ≥100%, rounded to 5
    elif regime == "volatile":
    ...
    messages.append(
        f"Dynamic band: ±{dynamic_limit_pct}% ({regime} regime){_band_note}"
    )
    ...
    if regime == "extreme":
        ...
        messages.append(
            f"Step-change guard: {max_step_change_pct}% (2× {max_move:.0f}% worst day)"
        )
    ...
    if live_price > 0:
        ...
        if max_mid < min_max_mid:
            messages.append(
                f"Price rail ceiling raised to 15% above live price "
    ...
    if _rp["spare_adj"] != 0:
        ...
        if _spare_mid > 0:
            _spare_mid = max(0, _spare_mid + _rp["spare_adj"])
        if _spare_outer > 0:
            _spare_outer = max(0, _spare_outer + _rp["spare_adj"])
        if _spare_extreme > 0:
            _spare_extreme = max(0, _spare_extreme + _rp["spare_adj"])
    ...
    if vol.get("confidence") == "high":
        messages.append(
            f"Volatility: {vol['regime']} ({vol.get('std_dev_pct', 0):.1f}% daily std dev)"
        )
    elif vol.get("range_30d_pct", 0) > 0:
        messages.append(
            f"30d range: {vol['range_30d_pct']:.1f}% ({vol.get('regime', 'normal')})"
        )

    # Trade/volume info — show individual trades OR ticker volume
    if trades and trades.get("total_count", 0) > 0:
        messages.append(
            f"30d trades: {trades['total_count']} ({trades.get('volume_trend', '?')} volume)"
        )
    elif ticker_volume_30d > 0:
    ...
    try:
        ...
        _fee_est = get_suggested_transaction_fee(
            target_seconds=_fee_target_secs, cost=35_000_000
        )
        _fee_est_60 = get_suggested_transaction_fee(
            target_seconds=_fee_spike_secs, cost=35_000_000
        )
        if _fee_est.get("available"):
            ...
            if _fee_coin_raw >= 0.001:
                ...
                _fee_coin_size = round(
                    _math.ceil(_fee_coin_raw / _magnitude) * _magnitude, 6
                )
            else:
                _fee_coin_size = 0.001
            _smart_fee_xch = (
                round(_peak_fee_xch, 10)
                if _peak_fee_xch > 0
                else float(getattr(cfg, "TRANSACTION_FEE_XCH", Decimal("0.000001")))
            )
            messages.append(
    ...
    if _cat_xch_equiv is not None and _cat_xch_equiv > 0:
        ...
        _orig_cat_budget_xch = max(
            0.0, round(_cat_xch_equiv - _cat_topup_reserve_xch_equiv, 4)
        )
    else:
    ...
    _BUY_SPARE_OVERHEAD = (
        sum(
            (_spare_inner, _spare_mid, _spare_outer, _spare_extreme)[3 - i]
            * _size_mults[i]
            for i in range(4)
        )
        if _buy_ladder_reversed
        else _SPARE_OVERHEAD
    )
    ...
    if _avail_xch > 0 and _trading_xch >= (_MIN_OFFER_XCH * 2) and _target_n > 0:
        ...
        def _solve_base_xch(n):
            _den = max(
                1e-9, (n * _BUY_TIER_FACTOR + _BUY_SPARE_OVERHEAD) * _CP_HEADROOM_MULT
            )
        ...
        _n_outer = (
            max(0, round(_n_final * _count_dist[2]))
            if _n_final >= 4 and _max_tiers >= 3
            else 0
        )
        _n_extreme = (
            max(0, round(_n_final * _count_dist[3]))
            if _n_final >= 5 and _max_tiers == 4
            else 0
        )
        ...
        _smart_extreme = (
            round(_base_size * _size_mults[3], 4) if _max_tiers == 4 else 0.0
        )
        ...
        if _rp["max_offers_mult"] != 1.0:
            ...
            _smart_n_outer = (
                max(0, round(_adj_n * _count_dist[2]))
                if _adj_n >= 4 and _max_tiers >= 3
                else 0
            )
            _smart_n_extreme = (
                max(0, round(_adj_n * _count_dist[3]))
                if _adj_n >= 5 and _max_tiers == 4
                else 0
            )
            _smart_n_mid = max(
                1, _adj_n - _smart_n_inner - _smart_n_outer - _smart_n_extreme
            )
        if _rp["inner_tier_mult"] != 1.0:
            ...
            _smart_inner = min(
                _inner_cap, round(_smart_inner * _rp["inner_tier_mult"], 4)
            )
        if _rp["tier_size_mult"] != 1.0:
            ...
            _smart_mid = (
                max(_MIN_OFFER_XCH, round(_smart_mid * _tsm, 4))
                if _smart_mid > 0
                else 0.0
            )
            _smart_outer = (
                max(_MIN_OFFER_XCH, round(_smart_outer * _tsm, 4))
                if _smart_outer > 0
                else 0.0
            )
            _smart_extreme = (
                max(_MIN_OFFER_XCH, round(_smart_extreme * _tsm, 4))
                if _smart_extreme > 0
                else 0.0
            )
        ...
        if _apply_shock_floor:
            ...
            _spare_inner = (
                max(_spare_inner, _math_spare.ceil(_smart_n_inner * 0.5))
                if _smart_n_inner > 0
                else _spare_inner
            )
            _spare_mid = (
                max(_spare_mid, _math_spare.ceil(_smart_n_mid * 0.5))
                if _smart_n_mid > 0
                else _spare_mid
            )
            _spare_outer = (
            ...
            if _pre_shock != _post_shock:
                print(
                    f"[SMART_DEFAULTS] Price shock spare floor applied: "
        ...
        if _post_shock_total_xch > _trading_xch > 0:
            ...
            _smart_outer = (
                max(_MIN_OFFER_XCH, round(_base_size * _size_mults[2], 4))
                if _max_tiers >= 3
                else 0.0
            )
            _smart_extreme = (
                max(_MIN_OFFER_XCH, round(_base_size * _size_mults[3], 4))
                if _max_tiers == 4
                else 0.0
            )
            print(
                f"[SMART_DEFAULTS] Post-shock XCH rescale ×{_rescale:.4f}: "
        ...
        if mid_price and mid_price > 0 and _avail_cat > 0:
            ...
            _cat_budget = max(
                0.0, _avail_cat * 0.98 - _sniper_cat_prep - _topup_cat_prep
            )
            if _total_cat_prep > _cat_budget:
                ...
                _smart_outer = (
                    max(_MIN_OFFER_XCH, round(_smart_outer * _cat_scale, 4))
                    if _smart_outer > 0
                    else 0.0
                )
                _smart_extreme = (
                    max(_MIN_OFFER_XCH, round(_smart_extreme * _cat_scale, 4))
                    if _smart_extreme > 0
                    else 0.0
                )
                ...
                print(
                    f"[SMART_DEFAULTS] CAT prep clamp triggered: scale={_cat_scale:.3f}, "
        ...
        _topup_buffer_xch = max(
            0.0, round(_avail_xch - _fee_pool_xch - _sniper_pool_xch - _trading_xch, 4)
        )
    ...
    if _smart_trade_size > 0 and _smart_max_buy > 0 and _avail_xch > 0:
        ...
        if _rp["coin_prep_adj"] != 0.0:
            coin_prep_multiplier = max(
                1.0,
                min(
                    2.5, round((coin_prep_multiplier + _rp["coin_prep_adj"]) * 2) / 2.0
                ),
            )
        print(
            f"[SMART_DEFAULTS v2] Coin prep multiplier: {coin_prep_multiplier} "
    ...
    if _smart_inner > 0:
        ...
        _launcher_buy_budget = max(
            0.0, _avail_xch - _fee_pool_xch - _sniper_pool_xch - _topup_buffer_xch
        )
        ...
        if _buy_pool_xch > _launcher_buy_budget and _buy_pool_xch > 0:
            ...
            _smart_outer = (
                max(_MIN_OFFER_XCH, round(_smart_outer * _buy_scale, 4))
                if _smart_outer > 0
                else 0.0
            )
            _smart_extreme = (
                max(_MIN_OFFER_XCH, round(_smart_extreme * _buy_scale, 4))
                if _smart_extreme > 0
                else 0.0
            )
            ...
            print(
                f"[SMART_DEFAULTS] Buy pool clamp triggered: scale={_buy_scale:.3f}, "
    ...
    if (
        ...
        if _tier_warning_budget <= 0:
            ...
            print(
                f"[SMART_DEFAULTS] Reserve ≥ 90% of wallet: dropped buy offers "
                f"({_pre_max_buy} → 0). reserve={_xch_reserve:.2f}, "
                f"xch_spendable={xch_spendable:.2f}"
            )
        elif (
            _ladder_sum > _safe_tier_budget
            and _safe_tier_budget > 0
            and _ladder_sum > 0
        ):
            ...
            if _new_trade_size < _PRACTICAL_MIN_BASE and _PRACTICAL_MIN_BASE > 0:
                ...
                if _smart_inner > 0:
                    _smart_inner = round(_smart_trade_size * 1.5, 4)
                if _smart_mid > 0:
                    _smart_mid = round(_smart_trade_size * 1.0, 4)
                if _smart_outer > 0:
                    _smart_outer = round(_smart_trade_size * 0.5, 4)
                if _smart_extreme > 0:
                    _smart_extreme = round(_smart_trade_size * 0.2, 4)
                messages.append(
                ...
                print(
                    f"[SMART_DEFAULTS] Tier-sum clamp (max_buy reduction): "
                ...
                if _smart_inner > 0:
                    _smart_inner = max(
                        _MIN_OFFER_XCH, round(_smart_inner * _tier_scale, 4)
                    )
                if _smart_mid > 0:
                    _smart_mid = max(_MIN_OFFER_XCH, round(_smart_mid * _tier_scale, 4))
                if _smart_outer > 0:
                    _smart_outer = max(
                        _MIN_OFFER_XCH, round(_smart_outer * _tier_scale, 4)
                    )
                if _smart_extreme > 0:
                    _smart_extreme = max(
                        _MIN_OFFER_XCH, round(_smart_extreme * _tier_scale, 4)
                    )
                messages.append(
                ...
                print(
                    f"[SMART_DEFAULTS] Tier-sum clamp: scale={_tier_scale:.3f}, "
    ...
    if _orig_xch_budget > 0 and _n_final > 0 and _smart_inner > 0 and _avail_xch > 0:
        ...
        if _buy_ladder_reversed:
            _smart_buy_inner = max(
                _MIN_OFFER_XCH, round(_buy_base_max * _size_mults[3], 4)
            )
            _smart_buy_mid = max(
                _MIN_OFFER_XCH, round(_buy_base_max * _size_mults[2], 4)
            )
            _smart_buy_outer = max(
                _MIN_OFFER_XCH, round(_buy_base_max * _size_mults[1], 4)
            )
            _smart_buy_extreme = max(
                _MIN_OFFER_XCH, round(_buy_base_max * _size_mults[0], 4)
            )
        else:
            _smart_buy_inner = max(
                _MIN_OFFER_XCH, round(_buy_base_max * _size_mults[0], 4)
    ...
    if _smart_buy_inner > 0 and _avail_xch > 0:
        ...
        if _f66_tier_xch > _f66_budget > 0:
            ...
            _smart_buy_inner = max(
                _MIN_OFFER_XCH, round(_smart_buy_inner * _f66_scale, 4)
            )
            _smart_buy_mid = max(_MIN_OFFER_XCH, round(_smart_buy_mid * _f66_scale, 4))
            _smart_buy_outer = (
                max(_MIN_OFFER_XCH, round(_smart_buy_outer * _f66_scale, 4))
                if _smart_buy_outer > 0
                else 0.0
            )
            _smart_buy_extreme = (
                max(_MIN_OFFER_XCH, round(_smart_buy_extreme * _f66_scale, 4))
    ...
    if (
        ...
        _sniper_cat_tokens = (
            round((_smart_sniper_size / mid_price) * _cp_hm_f64) * _smart_sniper_prep
        ...
        if _f64_sell_budget_xch > 0:
            ...
            _sell_denom = max(1e-9, (_sell_live_coeff + _sell_spare_coeff) * _cp_hm_f64)
            ...
            if _sell_base_max > _base_size * 1.05:
                ...
                def _f64_size_and_verify(base):
                    ...
                    so = (
                        max(_MIN_OFFER_XCH, round(base * _size_mults[2], 4))
                        if _max_tiers >= 3 and _smart_sell_outer > 0
                        else _smart_sell_outer
                    )
                    se = (
                        max(_MIN_OFFER_XCH, round(base * _size_mults[3], 4))
                ...
                _f64_sizes, _f64_total_cat = _f64_size_and_verify(_sell_base_max)
                ...
                if _sell_base_max > _base_size * 1.05:
                    ...
                    _smart_sell_outer, _smart_sell_extreme = (
                        _f64_sizes[2],
                        _f64_sizes[3],
                    )
        ...
        if _n_sell_cap > _smart_max_sell and _smart_sell_inner > 0:
            ...
            if _f64_expand_target > _smart_max_sell:
                ...
                def _f64_distribute(n):
                    ...
                    no = (
                        max(0, round(n * _count_dist[2]))
                        if n >= 4 and _max_tiers >= 3
                        else 0
                    )
                    ne = (
                        max(0, round(n * _count_dist[3]))
                        if n >= 5 and _max_tiers == 4
                        else 0
                    )
                    nm = max(1, n - ni - no - ne)
                    ...
                    so = (
                        max(_spare_outer, _math_f64.ceil(no * 0.5))
                        if no > 0
                        else _spare_outer
                    )
                    se = (
                        max(_spare_extreme, _math_f64.ceil(ne * 0.5))
                        if ne > 0
                        else _spare_extreme
                    )
                    _tiers = [
    ...
    if _avail_cat > 0 and mid_price and mid_price > 0 and _smart_sell_inner > 0:
        ...
        if _f65_total_cat > _avail_cat:
            ...
            _smart_sell_inner = max(
                _MIN_OFFER_XCH, round(_smart_sell_inner * _f65_scale, 4)
            )
            _smart_sell_mid = max(
                _MIN_OFFER_XCH, round(_smart_sell_mid * _f65_scale, 4)
            )
            _smart_sell_outer = (
                max(_MIN_OFFER_XCH, round(_smart_sell_outer * _f65_scale, 4))
                if _smart_sell_outer > 0
                else 0.0
            )
            _smart_sell_extreme = (
                max(_MIN_OFFER_XCH, round(_smart_sell_extreme * _f65_scale, 4))
                if _smart_sell_extreme > 0
                else 0.0
            )
            # Keep shared sizes in sync (pre-F62 callers read these)
            _smart_inner = _smart_sell_inner
    ...
    result = {
        ...
        "arb_alert_threshold_bps": int(
            round(arb_alert_threshold_bps)
        ),  # env key is ARB_ALERT_THRESHOLD_BPS
        "min_mid": min_mid,
        "max_mid": max_mid,
        ...
        **_smart_dbx_defaults(asset_id),
        ...
        "sell_extreme_size_xch": _smart_sell_extreme
        if _smart_sell_extreme > 0
        else None,
        ...
        "sniper_rearm_price_move_bps": int(
            round(
                max(
                    50,
                    min(
        ...
        "_data_sources": {
            ...
            "competitor_spread_bps": round(
                orderbook.get("competitor_spread_bps", 0), 0
            ),
            ...
            "fills_per_day": fills_per_day,
            "volume_trend": trades.get("volume_trend", "unknown")
            if trades
            else "unknown",
            "risk_level": risk_level,
            "messages": messages,
    ...
    print(
        f"[SMART_DEFAULTS v2] Offers: buy={_smart_max_buy}, sell={_smart_max_sell} | "
        ...
        + (
            ...
            if _sell_n_inner != _smart_n_inner
            else ""
        )
    ...
    print(
        f"[SMART_DEFAULTS v2] === Done! Spread: {api_server._bps_to_pct(base_spread_bps)}, "
    ...
    if liquidity_mode == "buy_only":
        ...
        _buy_ladder_xch = sum(
            (result.get(f"buy_{_t}_size_xch") or 0)
            * (result.get(f"buy_{_t}_tier_count") or 0)
        ...
        _sell_ladder_xch = sum(
            (result.get(f"sell_{_t}_size_xch") or 0)
            * (result.get(f"sell_{_t}_tier_count") or 0)
            for _t in ("inner", "mid", "outer", "extreme")
        )
    ...
    try:
        ...
        if _worst > 0:
            ...
            if _cur < _min_pos:
                ...
                print(
                    f"[SMART_DEFAULTS] Max position consistency clamp: "
                    f"{_cur} → {_min_pos} XCH "
