...
- [ ] Remaining compatibility/prerequisite-family staged funding/counts and actual unsigned costs must be derived from the verified snapshot. HTTP/native preview accepts no caller-supplied scope, plan, stage costs or funding. Execution must still adopt the frozen outputs and their ordering, rather than independently regenerating them from CLI overrides or a later market price.
