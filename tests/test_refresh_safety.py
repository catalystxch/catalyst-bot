...
def test_refresh_planner_allows_cancel_first_only_for_exact_bool_operator_request():
    ...
    with pytest.raises(TypeError, match="exact bool"):
        plan_refresh(
            [_parent("p1")], overlap_capacity=0, batch_size=1, operator_mass_cancel=1
        )
...
def test_parent_cannot_be_cancel_eligible_until_exact_confirmed_child_is_visible(
    isolated_database,
):
    ...
    database.bind_refresh_lineage("parent", "child")
    assert (
        database.refresh_parent_cancel_eligibility("parent", require_visible=True)[
            "eligible"
        ]
        is False
    )
    ...
    eligibility = database.refresh_parent_cancel_eligibility(
        "parent", require_visible=True
    )
    assert eligibility["eligible"] is True
    assert eligibility["child_trade_id"] == _sha("trade:child")


def test_refresh_lineage_binding_is_idempotent_and_rejects_cross_lineage_rows(
    isolated_database,
):
...
def test_visible_parent_selects_exact_next_generation_for_a_replacement_child(
    isolated_database,
):
    ...
    _prepare(
        "visible-child", parent_intent_id="parent", generation=resolved["generation"]
    )
...
def test_lineage_completion_requires_cancel_resolution_and_terminal_proof(
    isolated_database,
):
...
def test_real_task8_task9_completion_replays_one_exact_lineage_commit(
    isolated_database,
):
    ...
    assert (
        database.get_refresh_lineage_commit_for_child("child") == first_commit["commit"]
    )
    assert (
        database.get_pending_refresh_lineage_parent_ids(
...
def test_refresh_blocker_requires_exact_repair_proof_before_latch_resolution(
    isolated_database,
):
    ...
    assert (
        database.resolve_refresh_lineage_blocker(
            operation_id=operation_id,
            cohort_trade_ids=[repair_trade],
        )["idempotent"]
        is True
...
def test_requote_resumes_eligible_lineage_before_zero_spare_capacity_gate(
    ...
    parent_trade = _sha("trade:parent")
    monkeypatch.setattr(
        "offer_manager.get_open_offers", lambda **_kwargs: [{"trade_id": parent_trade}]
    )
...
def test_pending_lineage_query_finds_parent_when_only_its_live_child_remains(
    ...
    database.record_offer_intent_visibility(
        "live-child", publication_identity="registry:child"
    )
