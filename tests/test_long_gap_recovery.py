...
def test_begin_recovery_epoch_atomically_trips_latch_and_fences_late_publication(
    ...
    assert (
        db.complete_publication_outbox(
...
def test_recovery_epoch_fences_delayed_claimed_cancel_completion(isolated_database):
    ...
    assert (
        isolated_database.claim_offer_cancel_effect(
            operation_id=operation_id,
            trade_id=trade_id,
            attempt=1,
...
def test_quarantine_archives_immutable_evidence_without_clearing_authority(
    ...
    assert (
        ...
        == hashlib.sha256(archived["manifest_json"].encode("utf-8")).hexdigest()
    )
...
def test_bot_cycle_detects_discontinuity_before_first_mutation(monkeypatch):
    ...
    loop._runtime_recovery_coordinator = lambda decision, sample: (
        calls.append((decision.reason_code, sample))
...
def test_cancel_retry_does_not_hide_wall_clock_jump_during_sage_proof_pass():
    import bot_loop
    ...
    loop._enter_runtime_effect_phase = lambda phase: phase == "cancel"
    loop._runtime_recovery_coordinator = lambda decision, sample: (
        decisions.append((decision, sample))
        or {"allowed": False, "reason_code": "DURABLE_STATE_UNAVAILABLE"}
    )
    loop.offer_manager = _OfferManager()

    assert loop._run_cancel_retry_pass() is False
    assert decisions[0][0].reason_code == "WALL_CLOCK_JUMP"
    assert loop._runtime_recovery_baseline == _sample(Decimal("100"), NOW)
...
def test_runtime_recovery_reuses_ordered_startup_coordinator_and_retries_same_epoch(
    ...
    monkeypatch.setattr(
        ...
        lambda current_runtime: (
            events.append(("rotate", current_runtime.run_id)) or True
        ),
    )
...
def test_recovery_identity_revalidation_retries_same_windows_clock_tick(monkeypatch):
    ...
    sleeps = []
    monkeypatch.setattr(
        api_server.time, "sleep", lambda seconds: sleeps.append(seconds)
    )
...
def test_runtime_recovery_promotion_retries_with_new_append_only_attempt(
    ...
    monkeypatch.setattr(
        ...
        "record_runtime_recovery_pass",
        lambda **kwargs: (
            pass_attempts.append(kwargs) or {"attempt_number": len(pass_attempts)}
        ),
    )
...
def test_released_recovery_epoch_can_be_adopted_by_exact_successor_and_promoted(
    ...
    assert (
        adopted["record"]["predecessor_lease_snapshot_sha256"]
...
def test_active_expired_recovery_epoch_requires_dead_owner_proof_for_adoption(
    ...
    monkeypatch.setattr(
        ...
        lambda: takeover_now.isoformat(timespec="microseconds").replace("+00:00", "Z"),
    )
    common = {
        "recovery_id": epoch["recovery_id"],
        "successor_owner_run_id": "run-successor",
...
def test_ordered_startup_resumes_released_frozen_recovery_epoch(
    ...
    binding = mutation_gate.WalletIdentityBinding(
        ...
        kind="BLS",
        has_secrets=True,
        bound_at_utc=(base - timedelta(seconds=2))
        .isoformat(timespec="microseconds")
        .replace("+00:00", "Z"),
    )
...
def test_gap_fences_late_publication_retry_and_unresolve_callbacks(isolated_database):
    ...
    assert (
        db.unresolve_publication_outbox(
            **common,
...
def test_empty_quarantine_proof_requires_actual_authoritative_history_read(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_wallet_identity",
        lambda: (
            calls.append("identity") or (_ for _ in ()).throw(RuntimeError("offline"))
        ),
    )
    monkeypatch.setattr(
        wallet,
        ...
        lambda **_kwargs: (
            calls.append("history") or (_ for _ in ()).throw(RuntimeError("offline"))
        ),
    )
    requirements = {
...
def test_fresh_authoritative_empty_history_proves_truly_empty_quarantine(monkeypatch):
    ...
    monkeypatch.setattr(wallet, "get_authoritative_offer_history", lambda **_kwargs: [])
    requirements = {
...
def test_quarantine_api_streams_only_one_bounded_extra_byte_without_content_length(
    ...
    monkeypatch.setattr(
        api_server,
        downstream_name,
        lambda _payload: downstream_calls.append(True) or {"success": True},
    )
    with api_server.app.test_request_context(
        path,
        method="POST",
...
def test_quarantine_status_reports_current_latch_state_not_capture_constant(
    monkeypatch,
):
    import api_server
    ...
    with api_server.app.test_request_context(
        ...
        response = api_server.api_safety_quarantine_status("quarantine:" + "7" * 64)
