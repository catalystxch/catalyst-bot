...
class ProbeAnchorTests(unittest.TestCase):
    ...
    def test_wallet_missing_submitted_cancel_is_left_for_fence_settlement(self):
        loop = bot_loop.BotLoop()
        now_ts = 1000.0
        created_at = datetime.fromtimestamp(now_ts - 180, timezone.utc).isoformat()
        expires_at = datetime.fromtimestamp(now_ts - 60, timezone.utc).isoformat()
                ...
                return_value=[
                    {
                        ...
                        "operation_id": "cancel:expired-sell",
                    }
                ],
                create=True,
            ),
        ...
        self.assertEqual(result["sell"], set())
        fake_reconciliation.reconcile_offer.assert_not_called()
